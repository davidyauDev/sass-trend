<section class="w-full bg-[#eef3f7] px-4 py-6 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-6xl space-y-5">
        ...
        <div class="rounded-[18px] border border-white/80 bg-white px-4 py-4 shadow-sm">
            <div class="grid gap-3 xl:grid-cols-[1.25fr_1.15fr_1.15fr_1.25fr_auto]">
                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-800">Período de tiempo</label>
                ...
                <div>
                    <label class="mb-2 block text-sm font-semibold text-slate-800">Tipos de usuario</label>
                ...
                <div class="flex items-end">
                    <button
                        ...
                        class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-[10px] bg-violet-600 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-700 xl:w-auto"
                    >
        ...
        <div class="grid gap-4 rounded-[18px] border border-white/80 bg-white px-8 py-5 shadow-sm md:grid-cols-2 xl:grid-cols-3">
            <div>
        ...
        <div class="rounded-[18px] border border-white/80 bg-white px-4 py-4 shadow-sm">
            <div class="flex flex-col gap-4 border-b border-slate-100 px-2 pb-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-[1.05rem] font-semibold text-slate-900">
                    <div>
                        Ventas totales:
                ...
                <table class="min-w-full border-separate border-spacing-0">
                    ...
                    <tbody class="bg-white text-sm text-slate-700">
                        @forelse ($this->reportRows as $row)
                            <tr>
                                <td class="border-b border-slate-200 px-4 py-4">{{ $row['professional_name'] }}</td>
                                <td class="border-b border-slate-200 px-4 py-4">{{ $this->money($row['sales_total']) }}</td>
                                <td class="border-b border-slate-200 px-4 py-4">{{ $this->money($row['commission_amount']) }}</td>
                            </tr>
                        @empty
                            <tr>
